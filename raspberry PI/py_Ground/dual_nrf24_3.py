...
file_name = "cFS_to_LOG.bin"

def data_extract(data):
    """ data repalcing by \x00 *10 with @ byte """
    return data.replace(b'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00', b'@')

def add_brackets(data):
    return b'<' + data + b'>'
...
def radio_1_listener():
    while True:
        if radio_1.available():
            ...
            received_payload = received_payload.replace(b'#', b'\x00')
...
def radio_2_sender():
    while True:
        if not udp_to_radio_queue.empty():
            ...
            extracted_message = data_extract(message)
            payload = add_brackets(extracted_message)
            radio_2.write(payload)
            print(f"Sent raw data: {payload}")
