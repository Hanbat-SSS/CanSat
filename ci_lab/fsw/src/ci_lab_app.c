...
void CI_LAB_TaskInit(void)
{
    ...
    status = OS_OpenCreate(&CI_LAB_Global.SerialFd, "/dev/ttyUSB0", OS_FILE_FLAG_NONE, OS_READ_WRITE);
    if (status != OS_SUCCESS)
    {
        CFE_EVS_SendEvent(CI_LAB_SERIALOPEN_ERR_EID, CFE_EVS_EventType_ERROR, "CI: open serial port failed = %d",
                          (int)status);
    }
    else
    {
        OS_printf("CI_LAB: Serial port opened successfully.\n");
        CI_LAB_Global.SerialConnected = true;
    }
...
void CI_LAB_ReadUpLink(void)
{
    ...
    for (i = 0; i <= CI_LAB_MAX_INGEST_PKTS; i++)
    {
        ...
        OsStatus = OS_read(CI_LAB_Global.SerialFd, CI_LAB_Global.NetBufPtr, CI_LAB_Global.NetBufSize);
        if (OsStatus > 0)
        {
            CFE_ES_PerfLogEntry(CI_LAB_SERIAL_RCV_PERF_ID);
            ...
            CFE_ES_PerfLogExit(CI_LAB_SERIAL_RCV_PERF_ID);
